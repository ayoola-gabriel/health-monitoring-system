...
uint8_t calib_progress;

uint8_t max32664::calibProgress(){
...
bool  max32664::writeMax301xxReg(uint8_t regAddr, uint8_t val){
  ...
  uint8_t ret = writeByte(0x40, 0x03, regAddr, val);
  delay(10);
  if(!ret){
    return false;
  }else return true;
...
uint8_t  max32664::readMax301xxReg(uint8_t regAddr){
  ...
  uint8_t ret = readByte(0x41, 0x03, regAddr);
  delay(10);
  if(!ret){
    return ERR_UNKNOWN;
  }else return ret;
